...
class TestAuthenticatedBookApi(APITestCase):
    def setUp(self) -> None:
        ...
        book = Book.objects.create(
            title="test", author="test", cover="SOFT", inventory=10, fee="1.00"
        )
    ...
    def test_list_borrowings(self):
        ...
        serializer = BorrowingListSerializer(
            Borrowing.objects.filter(user_id=1), many=True
        )
    ...
    def test_create_borrowing(self):
        data = {"expected_return": "2023-10-10", "book": 1}
...
class TestAdminBorrowingApi(APITestCase):
    ...
    def test_return_borrowing(self):
        ...
        self.assertEqual(
            Borrowing.objects.get(id=1).actual_return, datetime.date.today()
        )
